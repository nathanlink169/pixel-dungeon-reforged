...
public class Armor extends EquipableItem {
	...
	public boolean doUnequip( Hero hero, boolean collect, boolean single ) {
		if (super.doUnequip( hero, collect, single )) {
			...
			ArmorerLightCharger lightCharger = hero.buff(ArmorerLightCharger.class);
			if (lightCharger != null) {
				Light light = hero.buff (Light.class);

				if (light != null && light.visualcooldown() == lightCharger.expectedDurationRemaining) {
					light.detach();
				}
	...
	public int DRMax(int lvl){
		...
		if (Dungeon.hero != null && Dungeon.hero.subClass == HeroSubClass.ARMORER) {
			...
			if (Dungeon.hero.hasTalent(Talent.ARMOR_MOD_EMERGENCY_DEFENSE)) {
				float hpThreshold = 0.15f;
				switch (Dungeon.hero.pointsInTalent(Talent.ARMOR_MOD_EMERGENCY_DEFENSE)) {
					case 2:
						hpThreshold = 0.2f;
						break;
					case 3:
		...
		if (Dungeon.isChallenged(Challenges.NO_ARMOR)){
			return 1 + tier + lvl + augment.defenseFactor(lvl) + artificerDamageReduction;
		}

		int max = tier * (2 + lvl) + augment.defenseFactor(lvl);
		if (lvl > max){
			return ((lvl - max)+1)/2 + artificerDamageReduction;
		} else {
			return max + artificerDamageReduction;
		}
	}
	...
	public int DRMin(int lvl){
		...
		if (Dungeon.isChallenged(Challenges.NO_ARMOR)){
			return 0 + artificerDamageReduction;
		}

		int max = DRMax(lvl);
		if (lvl >= max){
			return (lvl - max) + artificerDamageReduction;
		} else {
	...
	public static class ArmorerLightCharger extends Buff implements ActionIndicator.Action {
		public int cooldown = 0;
		public int expectedDurationRemaining = 0;
		...
		public String actionName() {
			return Messages.get(Armor.class, "light");
		}

		@Override
		public int actionIcon() {
			return HeroIcon.ARMORER_LIGHT;
		}
		...
		public Visual secondaryVisual() {
			...
			txt.text( Integer.toString((int)cooldown) );
