...
public class Corrosion extends Armor.Glyph implements CombatModifier.OnHitEffect {

	private static ItemSprite.Glowing BLACK = new ItemSprite.Glowing( 0x000000 );

	@Override
	public void onHit(AttackContext context, int finalDamage) {
		float procChance = 1/10f * procChanceMultiplier(context.defender);
		if ( Random.Float() < procChance ) {
			int pos = context.defenderPosition;
