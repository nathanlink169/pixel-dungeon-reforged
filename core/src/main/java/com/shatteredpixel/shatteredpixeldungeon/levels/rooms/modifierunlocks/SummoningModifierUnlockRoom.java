...
public class SummoningModifierUnlockRoom extends ModifierUnlockRoom {
	...
	public void paint(Level level) {
		...
		Painter.set(level, new Point(right - 1, bottom - 1), Terrain.PEDESTAL);
		...
		for (Point p : getPoints()){
			...
			if (level.map[cell] == Terrain.EMPTY){
				level.setTrap(new SummoningTrap().OverrideMobCount(5).reveal(), cell);
				Painter.set(level, cell, Terrain.TRAP);
			}
		}

		level.drop(new ChallengeCup(), level.pointToCell(new Point(right - 1, bottom - 1)));
