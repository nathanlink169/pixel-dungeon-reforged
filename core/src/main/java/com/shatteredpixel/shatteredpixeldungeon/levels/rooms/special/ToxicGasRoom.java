...
public class ToxicGasRoom extends SpecialRoom {
	...
	public void paint(Level level) {
		...
		//then 1 chest with regular gold (no mimics, chance for trinket catalyst)
		//then 1 chest with 33% change regular gold, 33% chance trinket, 33% chance ring
		...
		Item item1 = level.findPrizeItem(TrinketCatalyst.class);
		if (item1 == null) item1 = new Gold().random();
		level.drop(item1, goldPositions.remove(0)).type = Heap.Type.CHEST;

		Item item2 = Generator.random(Random.oneOf(
				Generator.Category.GOLD,
				Generator.Category.RING,
				Generator.Category.ARTIFACT
		));
		if (item2 instanceof Gold) {
			((Gold)item2).random();
		}
		level.drop(item2, goldPositions.remove(0)).type = Heap.Type.CHEST;
