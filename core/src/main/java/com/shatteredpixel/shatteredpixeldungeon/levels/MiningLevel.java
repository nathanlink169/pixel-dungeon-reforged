...
public class MiningLevel extends CavesLevel {
	...
	public String tilesTex() {
		switch (Blacksmith.Quest.Type()){
			...
			case Blacksmith.Quest.KOBOLD:
				return Assets.Environment.TILES_CAVES_KOBOLDS;
		}
		...
		if (super.build()){
			...
			if (Blacksmith.Quest.Type() == Blacksmith.Quest.KOBOLD) {
				for (int i = 0; i < length(); i++) {
					// Get rid of 3/4 of the ground decoration, as the magma should be sporadic
		...
		if (transition.type == LevelTransition.Type.BRANCH_ENTRANCE
			...
			if (!Blacksmith.Quest.bossBeaten()){
				switch (Blacksmith.Quest.Type()){
					...
					case Blacksmith.Quest.KOBOLD: warnText += "\n\n" + Messages.get(Blacksmith.class, "exit_warn_kobold"); break;
				}
			}
