...
public class Badges {
	...
	public enum Badge {
		...
		VICTORY                     ( 82 ),
		BOSS_CHALLENGE_GOO			( 83 ),
		BOSS_CHALLENGE_USURPER		( 88 ),
		BOSS_CHALLENGE_TENGU		( 84 ),
		...
		BOSS_SLAIN_3_CONSTRUCTOR,
		BOSS_SLAIN_3_ARMORER,
		BOSS_SLAIN_3_ALL_SUBCLASSES ( 104, BadgeType.GLOBAL ),
		BOSS_CHALLENGE_DM300		( 105 ),
		BOSS_CHALLENGE_KING			( 106 ),
	...
	static {
		...
		thirdBossSubclassBadges.put(HeroSubClass.CONSTRUCTOR, Badge.BOSS_SLAIN_3_CONSTRUCTOR);
		thirdBossSubclassBadges.put(HeroSubClass.ARMORER, Badge.BOSS_SLAIN_3_ARMORER);
	}
	...
	public static void validateBossSlain(Mob m) {
		...
		if (m instanceof Tengu) badge = Badge.BOSS_SLAIN_2;
		if (m instanceof DM300) badge = Badge.BOSS_SLAIN_3;
		if (m instanceof DwarfKing) badge = Badge.BOSS_SLAIN_4;
		
		if (badge != null) {
			local.add( badge );
			displayBadge( badge );
			
			if (badge == Badge.BOSS_SLAIN_GOO || badge == Badge.BOSS_SLAIN_RAT) {
	...
	public static void validateBossChallengeCompleted(){
		...
		switch (Dungeon.depth) {
			case 5:
				if (Dungeon.Boss1Version == 0) {
					badge = Badge.BOSS_CHALLENGE_GOO;
				} else if (Dungeon.Boss1Version == 1) {
					badge = Badge.BOSS_CHALLENGE_USURPER;
				}
				break;
			case 10:
				badge = Badge.BOSS_CHALLENGE_TENGU;
				break;
			case 15:
				badge = Badge.BOSS_CHALLENGE_DM300;
				break;
			case 20:
				badge = Badge.BOSS_CHALLENGE_KING;
				break;
			case 25:
	...
	public static List<Badge> GetVisibleBadges( boolean global ) {
		...
		return GetVisibleBadges(badges);
	}

	public static List<Badge> GetVisibleBadges( List<Badge> badges ) {
		List<Badge> visibleBadges = new ArrayList<>();
	...
	public static boolean CanShowBadge(Badge b) {
		if (b == Badge.MONSTERS_SLAIN_5) return isUnlocked(Badge.MONSTERS_SLAIN_4);
		if (b == Badge.MONSTERS_SLAIN_4) return isUnlocked(Badge.MONSTERS_SLAIN_3) && !isUnlocked(Badge.MONSTERS_SLAIN_5);
		if (b == Badge.MONSTERS_SLAIN_3) return isUnlocked(Badge.MONSTERS_SLAIN_2) && !isUnlocked(Badge.MONSTERS_SLAIN_4);
		if (b == Badge.MONSTERS_SLAIN_2) return isUnlocked(Badge.MONSTERS_SLAIN_1) && !isUnlocked(Badge.MONSTERS_SLAIN_3);
		if (b == Badge.MONSTERS_SLAIN_1) return !isUnlocked(Badge.MONSTERS_SLAIN_2);
		if (b == Badge.GOLD_COLLECTED_5) return isUnlocked(Badge.GOLD_COLLECTED_4);
		if (b == Badge.GOLD_COLLECTED_4) return isUnlocked(Badge.GOLD_COLLECTED_3) && !isUnlocked(Badge.GOLD_COLLECTED_5);
		if (b == Badge.GOLD_COLLECTED_3) return isUnlocked(Badge.GOLD_COLLECTED_2) && !isUnlocked(Badge.GOLD_COLLECTED_4);
		if (b == Badge.GOLD_COLLECTED_2) return isUnlocked(Badge.GOLD_COLLECTED_1) && !isUnlocked(Badge.GOLD_COLLECTED_3);
		if (b == Badge.GOLD_COLLECTED_1) return !isUnlocked(Badge.GOLD_COLLECTED_2);
		if (b == Badge.ITEM_LEVEL_5) return isUnlocked(Badge.ITEM_LEVEL_4);
		if (b == Badge.ITEM_LEVEL_4) return isUnlocked(Badge.ITEM_LEVEL_3) && !isUnlocked(Badge.ITEM_LEVEL_5);
		if (b == Badge.ITEM_LEVEL_3) return isUnlocked(Badge.ITEM_LEVEL_2) && !isUnlocked(Badge.ITEM_LEVEL_4);
		if (b == Badge.ITEM_LEVEL_2) return isUnlocked(Badge.ITEM_LEVEL_1) && !isUnlocked(Badge.ITEM_LEVEL_3);
		if (b == Badge.ITEM_LEVEL_1) return !isUnlocked(Badge.ITEM_LEVEL_2);
		if (b == Badge.LEVEL_REACHED_5) return isUnlocked(Badge.LEVEL_REACHED_4);
		if (b == Badge.LEVEL_REACHED_4) return isUnlocked(Badge.LEVEL_REACHED_3) && !isUnlocked(Badge.LEVEL_REACHED_5);
		if (b == Badge.LEVEL_REACHED_3) return isUnlocked(Badge.LEVEL_REACHED_2) && !isUnlocked(Badge.LEVEL_REACHED_4);
		if (b == Badge.LEVEL_REACHED_2) return isUnlocked(Badge.LEVEL_REACHED_1) && !isUnlocked(Badge.LEVEL_REACHED_3);
		if (b == Badge.LEVEL_REACHED_1) return !isUnlocked(Badge.LEVEL_REACHED_1);
		if (b == Badge.STRENGTH_ATTAINED_5) return isUnlocked(Badge.STRENGTH_ATTAINED_4);
		if (b == Badge.STRENGTH_ATTAINED_4) return isUnlocked(Badge.STRENGTH_ATTAINED_3) && !isUnlocked(Badge.STRENGTH_ATTAINED_5);
		if (b == Badge.STRENGTH_ATTAINED_3) return isUnlocked(Badge.STRENGTH_ATTAINED_2) && !isUnlocked(Badge.STRENGTH_ATTAINED_4);
		if (b == Badge.STRENGTH_ATTAINED_2) return isUnlocked(Badge.STRENGTH_ATTAINED_1) && !isUnlocked(Badge.STRENGTH_ATTAINED_3);
		if (b == Badge.STRENGTH_ATTAINED_1) return !isUnlocked(Badge.STRENGTH_ATTAINED_2);
		if (b == Badge.FOOD_EATEN_5) return isUnlocked(Badge.FOOD_EATEN_4);
		if (b == Badge.FOOD_EATEN_4) return isUnlocked(Badge.FOOD_EATEN_3) && !isUnlocked(Badge.FOOD_EATEN_5);
		if (b == Badge.FOOD_EATEN_3) return isUnlocked(Badge.FOOD_EATEN_2) && !isUnlocked(Badge.FOOD_EATEN_4);
		if (b == Badge.FOOD_EATEN_2) return isUnlocked(Badge.FOOD_EATEN_1) && !isUnlocked(Badge.FOOD_EATEN_3);
		if (b == Badge.FOOD_EATEN_1) return !isUnlocked(Badge.FOOD_EATEN_2);
		if (b == Badge.ITEMS_CRAFTED_5) return isUnlocked(Badge.ITEMS_CRAFTED_4);
		if (b == Badge.ITEMS_CRAFTED_4) return isUnlocked(Badge.ITEMS_CRAFTED_3) && !isUnlocked(Badge.ITEMS_CRAFTED_5);
		if (b == Badge.ITEMS_CRAFTED_3) return isUnlocked(Badge.ITEMS_CRAFTED_2) && !isUnlocked(Badge.ITEMS_CRAFTED_4);
		if (b == Badge.ITEMS_CRAFTED_2) return isUnlocked(Badge.ITEMS_CRAFTED_1) && !isUnlocked(Badge.ITEMS_CRAFTED_3);
		if (b == Badge.ITEMS_CRAFTED_1) return !isUnlocked(Badge.ITEMS_CRAFTED_2);
		if (b == Badge.BOSS_SLAIN_4) return isUnlocked(Badge.BOSS_SLAIN_3);
		if (b == Badge.BOSS_SLAIN_3) return isUnlocked(Badge.BOSS_SLAIN_2);
		if (b == Badge.BOSS_SLAIN_2) return isUnlocked(Badge.BOSS_SLAIN_GOO) || isUnlocked(Badge.BOSS_SLAIN_RAT);
		if (b == Badge.RESEARCHER_5) return isUnlocked(Badge.RESEARCHER_4);
		if (b == Badge.RESEARCHER_4) return isUnlocked(Badge.RESEARCHER_3) && !isUnlocked(Badge.RESEARCHER_5);
		if (b == Badge.RESEARCHER_3) return isUnlocked(Badge.RESEARCHER_2) && !isUnlocked(Badge.RESEARCHER_4);
		if (b == Badge.RESEARCHER_2) return isUnlocked(Badge.RESEARCHER_1) && !isUnlocked(Badge.RESEARCHER_3);
		if (b == Badge.RESEARCHER_1) return !isUnlocked(Badge.RESEARCHER_2);
		if (b == Badge.HIGH_SCORE_5) return isUnlocked(Badge.HIGH_SCORE_4);
		if (b == Badge.HIGH_SCORE_4) return isUnlocked(Badge.HIGH_SCORE_3) && !isUnlocked(Badge.HIGH_SCORE_5);
		if (b == Badge.HIGH_SCORE_3) return isUnlocked(Badge.HIGH_SCORE_2) && !isUnlocked(Badge.HIGH_SCORE_4);
		if (b == Badge.HIGH_SCORE_2) return isUnlocked(Badge.HIGH_SCORE_1) && !isUnlocked(Badge.HIGH_SCORE_3);
		if (b == Badge.HIGH_SCORE_1) return !isUnlocked(Badge.HIGH_SCORE_2);
		if (b == Badge.GAMES_PLAYED_5) return isUnlocked(Badge.GAMES_PLAYED_4);
		if (b == Badge.GAMES_PLAYED_4) return isUnlocked(Badge.GAMES_PLAYED_3) && !isUnlocked(Badge.GAMES_PLAYED_5);
		if (b == Badge.GAMES_PLAYED_3) return isUnlocked(Badge.GAMES_PLAYED_2) && !isUnlocked(Badge.GAMES_PLAYED_4);
		if (b == Badge.GAMES_PLAYED_2) return isUnlocked(Badge.GAMES_PLAYED_1) && !isUnlocked(Badge.GAMES_PLAYED_3);
		if (b == Badge.GAMES_PLAYED_1) return !isUnlocked(Badge.GAMES_PLAYED_2);
		if (b == Badge.CHAMPION_3) return isUnlocked(Badge.CHAMPION_2);
		if (b == Badge.CHAMPION_2) return isUnlocked(Badge.CHAMPION_1) && !isUnlocked(Badge.CHAMPION_2);
		if (b == Badge.CHAMPION_1) return isUnlocked(Badge.VICTORY) && !isUnlocked(Badge.CHAMPION_1);
		if (b == Badge.BOSS_CHALLENGE_GOO || b == Badge.BOSS_SLAIN_RAT) return isUnlocked(Badge.BOSS_SLAIN_GOO);
		if (b == Badge.BOSS_CHALLENGE_USURPER) return isUnlocked(Badge.BOSS_SLAIN_RAT);
		if (b == Badge.BOSS_CHALLENGE_TENGU) return isUnlocked(Badge.BOSS_SLAIN_2);
		if (b == Badge.BOSS_CHALLENGE_DM300) return isUnlocked(Badge.BOSS_SLAIN_3);
		if (b == Badge.BOSS_CHALLENGE_KING) return isUnlocked(Badge.BOSS_SLAIN_4);
		if (b == Badge.BOSS_CHALLENGE_5) return isUnlocked(Badge.VICTORY);
		if (b == Badge.PACIFIST_ASCENT) return isUnlocked(Badge.HAPPY_END);
		if (b == Badge.TAKING_THE_MICK) return isUnlocked(Badge.VICTORY);
		if (b == Badge.BOSS_SLAIN_1_ALL_CLASSES) return isUnlocked(Badge.BOSS_SLAIN_GOO);
		if (b == Badge.BOSS_SLAIN_3_ALL_SUBCLASSES) return isUnlocked(Badge.BOSS_SLAIN_3);
		if (b == Badge.RODNEY) return isUnlocked(Badge.BOSS_SLAIN_4);
		if (b == Badge.HAPPY_END_REMAINS || b == Badge.HAPPY_END) return isUnlocked(Badge.VICTORY);
		if (b == Badge.MANY_BUFFS) return isUnlocked(Badge.BOSS_SLAIN_GOO);
		if (b == Badge.VICTORY) return isUnlocked(Badge.BOSS_SLAIN_4);
		if (b == Badge.MASTERY_COMBO) return isUnlocked(Badge.BOSS_SLAIN_2);
		if (b == Badge.ALL_BAGS_BOUGHT) return isUnlocked(Badge.BOSS_SLAIN_3);
		if (b == Badge.DEATH_FROM_ENEMY_MAGIC) return isUnlocked(Badge.BOSS_SLAIN_GOO) && !Badges.isUnlocked(Badge.DEATH_FROM_ALL);
		if (b == Badge.DEATH_FROM_FIRE ||
		    b == Badge.DEATH_FROM_GAS ||
		    b == Badge.DEATH_FROM_HUNGER ||
		    b == Badge.DEATH_FROM_POISON ||
		    b == Badge.DEATH_FROM_FALLING ||
		    b == Badge.DEATH_FROM_FRIENDLY_MAGIC ||
		    b == Badge.DEATH_FROM_SACRIFICE ||
		    b == Badge.DEATH_FROM_GRIM_TRAP) return !Badges.isUnlocked(Badge.DEATH_FROM_ALL);
