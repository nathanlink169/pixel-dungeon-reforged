...
public class Sungrass extends Plant {
	...
	public void activate( Char ch ) {
		...
		if (ch != null){
			if (ch instanceof Hero && ((Hero) ch).subClass == HeroSubClass.WARDEN) {
				Buff.affect(ch, Healing.class).setHeal(ch.GetMaxHP(), 0, 1);
			} else {
				Buff.affect(ch, Health.class).boost(ch.GetMaxHP());
			}
		}
	...
	public static class Health extends Buff {
		...
		public boolean act() {
			...
			partialHeal += (40 + target.GetMaxHP())/150f;
		...
		public float iconFadePercent() {
			return Math.max(0, (target.GetMaxHP() - level) / (float)target.GetMaxHP());
		}
