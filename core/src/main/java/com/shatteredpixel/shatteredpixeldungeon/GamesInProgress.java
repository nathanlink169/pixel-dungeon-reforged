...
public class GamesInProgress {
	...
	public static ArrayList<Info> checkAll(){
		...
		switch (SPDSettings.gamesInProgressSort()){
			...
			case "last_played":
				...
				break;
			case "difficulty":
				Collections.sort(result, difficultyComparator);
				break;
		}
	...
	public static final Comparator<GamesInProgress.Info> difficultyComparator = new Comparator<GamesInProgress.Info>() {
		@Override
		public int compare(GamesInProgress.Info lhs, GamesInProgress.Info rhs ) {
			if (rhs.level != lhs.level){
				return (int)Math.signum( rhs.difficulty - lhs.difficulty );
			} else {
				return lastPlayedComparator.compare(lhs, rhs);
			}
		}
