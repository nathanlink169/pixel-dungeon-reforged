...
public abstract class Char extends Actor {
	...
	public boolean attack( Char enemy, float dmgMulti, float dmgBonus, float accMulti, int damageType ) {
		...
		if (enemy.isInvulnerable(getClass())) {
			...
			if (this instanceof Hero){
				...
				// If we're on easy and it's a sneak attack, deal max damage
			...
			if (prep != null){
				dmg = prep.damageRoll(this, isMaxDamage);
			...
			if (effectiveDamage > 0) {
				...
				if (enemy.buff(Cursed.class) != null) {
					effectiveDamage *= 1.1f;
				}
	...
	public static boolean hit( Char attacker, Char defender, float accMulti, boolean magic ) {
		...
		if (defender.buff(Talent.ArtificerFoodEvasionBonus.class) != null) {
			if (((Hero)defender).pointsInTalent(Talent.QUICK_CALIBRATION) == 2 || Random.Int(4) == 0) {
				defStat = INFINITE_EVASION;
			}
		}
